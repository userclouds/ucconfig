...
type Resource struct {
	...
	// A unique ID for this resource that will be stable across environments and time. This is used
	// to produce a Terraform resource path. This can be an arbitrary ID, does not need to be a
	// UUID.
	ManifestID string `json:"manifest_id"`
	// A map of environment name to resource UUID within that environment. The key "__DEFAULT" can
	// be used to set a default UUID when creating this resource in a new environment.
	ResourceUUIDs map[string]string `json:"resource_uuids"`
	// A map of attributes to set on the Terraform resource.
	Attributes map[string]any `json:"attributes"`
}

func fromLiveResource(live *liveresource.Resource, envName string) Resource {
...
func rewriteManifestReferences(data any, currAttrPath string, forResource resourcetypes.ResourceType, mfest *Manifest, envName string, liveResources *[]liveresource.Resource) (any, error) {
	v := reflect.ValueOf(data)

	if v.Kind() == reflect.Pointer {
		return rewriteManifestReferences(v.Elem().Interface(), currAttrPath, forResource, mfest, envName, liveResources)
	}
	...
	if v.Kind() == reflect.String && forResource.References[currAttrPath] != "" {
		...
		for _, r := range mfest.Resources {
			if r.TerraformTypeSuffix == forResource.References[currAttrPath] && r.ResourceUUIDs[envName] == ref {
	...
	if v.Kind() == reflect.Array || v.Kind() == reflect.Slice {
		...
		for i := 0; i < v.Len(); i++ {
			...
			rewritten, err := rewriteManifestReferences(val, currAttrPath, forResource, mfest, envName, liveResources)
			if err != nil {
	...
	if v.Kind() == reflect.Map {
		...
		for _, key := range v.MapKeys() {
			...
			rewritten, err := rewriteManifestReferences(val, currAttrPath+"."+key.String(), forResource, mfest, envName, liveResources)
			if err != nil {
...
// ID.
func (mfest *Manifest) MatchLiveResources(ctx context.Context, liveResources *[]liveresource.Resource, envName string) error {
	...
	for manifestID, manifest := range unmatchedManifests {
		targetResourceID := manifest.ResourceUUIDs[envName]
		if targetResourceID == "" {
		...
		if resourceIndex, ok := unmatchedLiveResourceIndexes[targetResourceID]; ok && manifest.TerraformTypeSuffix == (*liveResources)[resourceIndex].TerraformTypeSuffix {
			...
			// environment-specific UUID we found
			if manifest.ResourceUUIDs[envName] == "" {
				manifest.ResourceUUIDs[envName] = targetResourceID
			}
		}
	}
	...
	for manifestID, manifest := range unmatchedManifests {
		// Skip entries where the manifest explicitly specified a UUID for this environment. If the
		...
		for _, resourceIndex := range unmatchedLiveResourceIndexes {
			...
			if manifestName == resourceName && manifest.TerraformTypeSuffix == (*liveResources)[resourceIndex].TerraformTypeSuffix {
				...
				(*liveResources)[resourceIndex].ManifestID = manifestID
				manifest.ResourceUUIDs[envName] = resourceID
				delete(unmatchedManifests, manifestID)
...
func (r *Resource) RewriteReferences(mfest *Manifest, envName string, liveResources *[]liveresource.Resource) error {
	for key, value := range r.Attributes {
		rewritten, err := rewriteManifestReferences(value, key, *resourcetypes.GetByTerraformTypeSuffix(r.TerraformTypeSuffix), mfest, envName, liveResources)
		if err != nil {
...
func (mfest *Manifest) Validate(envName string) error {
	for i, resource := range mfest.Resources {
		...
		if resource.ResourceUUIDs[envName] == "" && resource.ResourceUUIDs["__DEFAULT"] == "" {
			return ucerr.New(fmt.Sprintf("error validating resource at index %v: resource_uuids either must include a UUID for environment \"%s\", or it must include a __DEFAULT entry.", i, envName))
		}
	}
	return nil
}

func generateFromLiveResources(ctx context.Context, liveResources *[]liveresource.Resource, envName string) (Manifest, error) {
	...
	for _, r := range *liveResources {
		...
		resourceManifests = append(resourceManifests, fromLiveResource(&r, envName))
	}
	mfest := Manifest{
		Resources: resourceManifests,
	}
	for i := range mfest.Resources {
		if err := mfest.Resources[i].RewriteReferences(&mfest, envName, liveResources); err != nil {
...
func GenerateNewManifest(ctx context.Context, client *idp.Client, envName string) (Manifest, error) {
	liveResources, err := liveresource.GetLiveResources(ctx, client)
	if err != nil {
		return Manifest{}, ucerr.Wrap(err)
	}
	return generateFromLiveResources(ctx, &liveResources, envName)
}
