...
func GenerateNewManifest(ctx context.Context, idpClient *idp.Client, manifestPath string, envName string) {
	uclog.Infof(ctx, "Generating new manifest from live resource state...")

	mfest, err := manifest.GenerateNewManifest(ctx, idpClient, envName)
	if err != nil {
