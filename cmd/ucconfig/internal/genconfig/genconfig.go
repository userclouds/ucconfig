...
func genResourceConfig(resource *manifest.Resource, mfest *manifest.Manifest, envName string, liveResources *[]liveresource.Resource, body *hclwrite.Body) error {
	block := body.AppendNewBlock("resource", []string{"userclouds_" + resource.TerraformTypeSuffix, "manifestid-" + resource.ManifestID})
	var resourceUUID string
	if resource.ResourceUUIDs[envName] != "" {
		resourceUUID = resource.ResourceUUIDs[envName]
	} else {
...
func GenConfig(mfest *manifest.Manifest, envName string, liveResources *[]liveresource.Resource) (string, error) {
	// required providers
	...
	for _, resource := range mfest.Resources {
		if err := genResourceConfig(&resource, mfest, envName, liveResources, file.Body()); err != nil {
